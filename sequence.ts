...
function validateNumbers(nums: number[]):string {
    const error = nums.map((value, index)=>{
        if (!isFinite(value)) {
            return `Argument number ${index + 1} must be a valid number`;
        }
        if (value !== Math.floor(value)) {
            return `Argument number ${index + 1} must be an integer`;
        }
        return "";
    }).filter(x=>x).join("\n");
    if (error) {
    ...
    if (n > max - min + 1) {
        return `Argument number 1 must be less than or equal to the difference between the arguments 2 and 3`;
    }
    return "";
}
